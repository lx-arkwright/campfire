...
socket.on("welcome", (data) => {
  me = data.you;
  line(`You are ${me.name}. Seat: ${data.roomId}. Up to ${data.max} around this fire.`, "you");
  line("It's quiet here. Say hello, or wait for someone to wander in…", "system");
  input.focus();
});
...
socket.on("presence", (p) => {
  ...
  scene.dataset.brightness = p.brightness.toFixed(2);
  scene.style.setProperty("--brightness", p.brightness);
  renderSeats();
  renderStats();
});
...
socket.on("extinguished", () => {
  line("The others voted. You drift to a different fire…", "warn");
